...
export default function IncomeVouchers(props) {
    const [appLink,setApplink] = React.useState(AppUrl(`/secure/income/get-income-vouchers?`))
    ...
    const handleDeleteVoucher = async (id) => {
        ...
        .then(async (result) => {
            if (result.isConfirmed) {
                ...
                await axios.post(AppUrl(`/secure/income/delete-income-voucher`),formData)
    ...
    const handlePrintVoucher = async (voucher) => {
        window.open(AppUrl(`/income/print/`+voucher), "sales-invoice", "fullscreen=yes");
    }
                ...
                <Col>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Income Voucher List</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            <Row className="mb-2">
                                                ...
                                                (
                                                    ...
                                                    :(
                                                        Object.values(data.infos).map((info,index)=>(
                                                            <tr key={index}>
                                                                <td>{info.voucher_date}</td>
                                                                <td>
                                                                    Voucher No : {info.voucher} <br />
                                                                    Ref : {info.ref}
                                                                </td>
                                                                <td>
                                                                    {info.account_head.name}
                                                                    {
                                                                        info.customer ? (
                                                                            <p>{info.customer.name}<br /> {info.customer.phone}</p>
                                                                        ) :(``)
                                                                    }
                                                                    {
                                                                        info.supplier ? (
                                                                            <p>{info.supplier.name}<br /> {info.supplier.phone}</p>
                                                                        ) :(``)
                                                                    }
                                                                    {
                                                                        info.bank ? (
                                                                            <p>{info.bank.name}<br /> {info.bank.account_no}</p>
                                                                        ) :(``)
                                                                    }
                                                                </td>
                                                                ...
                                                                <td className="align-middle">
                                                                    <DropdownButton title="Action" className="btn-sm float-end">
                                                                        <Link className='dropdown-item' to={AppUrl(`/control/income/income-voucher-details/`+info.voucher)}>
                                                                            Details
                                                                        </Link>
                                                                        <Dropdown.Item eventKey="2" 
                                                                            onClick={() => handlePrintVoucher(info.voucher)}
                                                                        >Print</Dropdown.Item>
