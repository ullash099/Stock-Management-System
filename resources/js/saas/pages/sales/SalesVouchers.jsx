...
export default function SalesVouchers(props) {
    const [appLink,setApplink] = React.useState(AppUrl(`/secure/sales/get-sales-invoices?`))
                                    ...
                                    <Table striped bordered className='mb-0'>
                                        <thead>
                                            <tr>
                                                ...
                                                <th className="text-end" style={{ width : "20%" }}>Net Payable</th>
                                                <th className="text-end" style={{ width : "20%" }}>Total Receive</th>
                                                        ...
                                                        Object.values(data.infos).map((info,index)=>(
                                                            <tr key={index}>
                                                                ...
                                                                <td className='text-end'>
                                                                    {parseFloat(info.net_payable).toFixed(2)} <span dangerouslySetInnerHTML={{__html: currency.sign}}/>
                                                                </td>
                                                                <td className='text-end'>
                                                                    {parseFloat(info.cash+info.bank_amount).toFixed(2)} <span dangerouslySetInnerHTML={{__html: currency.sign}}/>
                                                                </td>
                                                                <td className="align-middle">
                                                                    <DropdownButton title="Action" className="btn-sm float-end">
                                                                        <Link className='dropdown-item' to={AppUrl(`/control/sales/invoice-details/`+info.invoice)}>
                                                                            Details
                                                                        </Link>
                                                                        <Dropdown.Item eventKey="2" 
                                                                            onClick={() => handlePrintInvoice(info.invoice)}
                                                                        >Print</Dropdown.Item>
                                                                        {
                                                                            !info.is_delivered ? 
                                                                                <Link className='dropdown-item' to={AppUrl(`/control/sales/delivery-invoice/`+info.invoice)}>
                                                                                    Delivery
                                                                                </Link>
                                                                            : (``)
                                                                        }
                                                                        <Link className='dropdown-item' to={AppUrl(`/control/sales/return-invoice/`+info.invoice)}>
                                                                            Return
                                                                        </Link>
                                                                        {/* <Dropdown.Item eventKey="3" 
                                                                            onClick={() => handleDeleteInvoice(info.id)}
