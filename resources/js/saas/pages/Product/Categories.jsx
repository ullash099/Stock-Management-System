...
export default function Categories(props) {
    ...
    const [appLink,setApplink] = React.useState(AppUrl(`/secure/product/get-categories?`))
        ...
        if(id){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    data.append('id', id)
                    await axios.post(AppUrl(`/secure/product/unblock-category`),data)
        ...
        if(blockItems.length > 0){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    await axios.post(AppUrl(`/secure/product/block-categories`),data)
                    .then(function (response) {
                        let info = response.data;
                        if(info.errors){
                            (info.errors).map((error)=>(
    ...
    const handelSavingInfo = async () => {
        ...
        await axios.post(AppUrl(`/secure/product/save-category`),data)
        .then(function (response) {            
            let info = response.data;
            if(info.errors){
                (info.errors).map((error)=>(
    ...
    const handleImport = async () => {
        ...
        await axios.post(AppUrl(`/secure/product/import-categories`),data)
        .then(function (response) {
            let info = response.data;
            if(info.errors){
                (info.errors).map((error)=>(
                        ...
                        <Card.Body>
                            ...
                            <Form.Group className="mb-2" controlId="description">
                                ...
                                <Form.Control type="text" 
                                    name="description" style={{ height: "100px", resize: "none" }}
                                    as="textarea"
                        ...
                        <Card.Body>
                            ...
                            <Row className="mb-2">
                                ...
                                <Col md={4} sm={12}>
                                    ...
                                    <Link to={AppUrl(`/secure/product/export-categories`)} target="_blank" className="btn btn-success float-end">
                                                    ...
                                                    :(
                                                        (data.infos).map((info,index)=>(
                                                            <tr key={index}>
                                                                ...
                                                                <td>
                                                                    {
                                                                        !info.deleted_at ? (<span>{info.description}</span>)
                                                                        : (<del>{info.description}</del>)
                                                                    }
                                                                </td>
