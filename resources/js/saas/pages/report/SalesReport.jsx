...
export default function SalesReport(props) {
    ...
    const handelGenerateReport = async () => {
        ...
        await axios.post(AppUrl(`/secure/report/generate-sales-report`),formData)
                ...
                Object.values(info.details).map((detail)=>{
                    if (Object.keys(detail.details).length > 0) {
                        Object.values(detail.details).map((d)=>{
                            total += d.price*(d.quantity-d.rtn_quantity)
                        })
                    }
                })
            }
    ...
    const handelPrint = async () => {
        window.open(AppUrl(`/report/print-sales-report?startData=`+srcData.startData+`&endData=`+srcData.endData), "sales-invoice", "fullscreen=yes");
    }
        ...
        <React.Fragment>
            <Row>
                <Col>
                    <Card>
                        <Card.Header>
                            <Card.Title>Sales Statement</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            <Form.Group>
                                <InputGroup>
                                                ...
                                                <Table striped bordered className='mb-0'>
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th className='text-end'>Quantity * Price</th>
                                                                                ...
                                                                                Invoice : {detail.invoice}
                                                                                {
                                                                                    ...
                                                                                    <React.Fragment>
                                                                                        ...
                                                                                        <p className='m-0 p-0'>Phone : {detail.customer.phone}</p>
                                                                                        {
                                                                                            ...
                                                                                            (<span dangerouslySetInnerHTML={{__html: detail.customer.customer_address}}/>) : (``)
                                                                                        }
                                                                                    </React.Fragment>
                                                                                    : (``)
                                                                                }
                                                                            ...
                                                                            Object.values(detail.details).map((d,i)=>(
                                                                                <tr key={i}>
                                                                                    ...
                                                                                    <td className='text-end'>
                                                                                        {parseFloat(d.quantity-d.rtn_quantity).toFixed(3)} {d.product.measurement ? ' /'+d.product.measurement.name: ``}
                                                                                        * 
                                                                                    ...
                                                                                    <td className='text-end'>
                                                                                        <span dangerouslySetInnerHTML={{__html: report.currency.sign}}/> 
                                                                                        {parseFloat(d.price*(d.quantity-d.rtn_quantity)).toFixed(2)}
                                                                                    </td>
                                                                                </tr>
                                                                            ))
                                                                        ): (``)
