...
export default function NewExpenseVoucher(props) {
    ...
    const handelSavingInfo = async () => {
        if(details.length > 0){
            ...
            data.append('expense_to', formData.expense_to)
            data.append('withdraw_from_bank', formData.withdraw_from_bank)
            ...
            await axios.post(AppUrl(`/secure/expense/save-expense-voucher`),data)
            .then(function (response) {
                ...
                if(info.errors){
                    ...
                    handelResetVoucher()
                    getDefaultData()
                    window.open(AppUrl(`/expense/print/`+info.voucher), "sales-invoice", "fullscreen=yes");
                }
                setSavingInfo(false);
            })
            .catch(function (error) {
                ...
                <Col sm={9}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title className='d-inline'>New Expense Voucher</Card.Title>
                            {
                                countRow <= 19 ? (
                        ...
                        <Card.Body>
                            ...
                            <Row>
                                <Col>
                                    <Form.Group className="mb-1" controlId="expense_to">
                                        <Form.Label className="m-0">Expense To</Form.Label>
                                        <Select
                                            value={expenseTo.filter(
                                                option => (formData.expense_to && option.value.toString() === (formData.expense_to).toString())
                                            )}
                                            options={expenseTo}
                                            onChange={
                                                option => {
                                                    setFormData({
                                                        ...formData,
                                                        expense_to : option ? option.value.toString() : `0`
                                                    })
