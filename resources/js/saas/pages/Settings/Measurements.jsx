...
export default function Measurements(props) {
    ...
    const [appLink,setApplink] = React.useState(AppUrl(`/secure/settings/get-measurements?`))
        ...
        if(id){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    data.append('id', id)
                    await axios.post(AppUrl(`/secure/settings/unblock-measurement`),data)
        ...
        if(blockItems.length > 0){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    await axios.post(AppUrl(`/secure/settings/block-measurements`),data)
                    .then(function (response) {
                        let info = response.data;
                        if(info.errors){
                            (info.errors).map((error)=>(
    ...
    const handelSavingInfo = async () => {
        ...
        await axios.post(AppUrl(`/secure/settings/save-measurement`),data)
        .then(function (response) {
            let info = response.data;
            if(info.errors){
                (info.errors).map((error)=>(
    ...
    const handleImport = async () => {
        ...
        await axios.post(AppUrl(`/secure/settings/import-measurements`),data)
        .then(function (response) {
            let info = response.data;
            if(info.errors){
                (info.errors).map((error)=>(
        ...
        <React.Fragment>
            <Row>
                <Col md={4} sm={12}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Measurement Unit Information</Card.Title>
                        </Card.Header>
                        <Card.Body>
                ...
                <Col md={8} sm={12}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Measurement Unit List</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            <Row className="mb-2">
                                ...
                                <Col md={4} sm={12}>
                                    ...
                                    <Link to={AppUrl(`/secure/settings/export-measurements`)} target="_blank" className="btn btn-success float-end">
            ...
            <Modal show={isImportModalOpen} onHide={handleImportModalClose.bind(this)} backdrop="static" keyboard={false}>
                ...
                <Modal.Body>
                    <strong>
                        ...
                        file must have this fields <br/>(Id, Name, Name (Bangla), Deleted by, Deleted at )
                    </strong>
