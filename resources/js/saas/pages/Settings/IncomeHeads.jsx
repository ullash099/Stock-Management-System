...
export default function IncomeHeads(props) {
    const searchFrom = [
        { value: 'na', label: 'NA' },
    ...
    const [appLink,setApplink] = React.useState(AppUrl(`/secure/settings/get-income-heads?`))
        ...
        if(id){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    data.append('id', id)
                    await axios.post(AppUrl(`/secure/settings/unblock-income-head`),data)
        ...
        if(blockItems.length > 0){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    await axios.post(AppUrl(`/secure/settings/block-income-heads`),data)
                    .then(function (response) {
                        let info = response.data;
                        if(info.errors){
                            (info.errors).map((error)=>(
    ...
    const handelSavingInfo = async () => {
        ...
        await axios.post(AppUrl(`/secure/settings/save-income-head`),data)
        .then(function (response) {
    ...
    const handleImport = async () => {
        ...
        await axios.post(AppUrl(`/secure/settings/import-income-heads`),data)
        .then(function (response) {
            let info = response.data;
            if(info.errors){
                (info.errors).map((error)=>(
        ...
        <React.Fragment>
            <Row>
                <Col md={4} sm={12}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Income Head Information</Card.Title>
                        </Card.Header>
                        <Card.Body>
                ...
                <Col md={8} sm={12}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Income Heads List</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            <Row className="mb-2">
                                ...
                                <Col md={4} sm={12}>
                                    ...
                                    <Link to={AppUrl(`/secure/settings/export-income-heads`)} target="_blank" className="btn btn-success float-end">
                                                    ...
                                                    :(
                                                        (data.infos).map((info,index)=>(
                                                            <tr key={index}>
                                                                <td>
                                                                    {
                                                                        [1,2,3].indexOf(info.id) >= 0 || info.deleted_at ? (``) :(
                                                                            <Form.Group>
                                                                                <Form.Check 
                                                                                    type="checkbox" 
