...
export default function ExpenseHeads(props) {
    const searchFrom = [
        { value: 'na', label: 'NA' },
    ...
    const [appLink,setApplink] = React.useState(AppUrl(`/secure/settings/get-expense-heads?`))
        ...
        if(id){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    data.append('id', id)
                    await axios.post(AppUrl(`/secure/settings/unblock-expense-head`),data)
        ...
        if(blockItems.length > 0){
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    ...
                    await axios.post(AppUrl(`/secure/settings/block-expense-heads`),data)
                    .then(function (response) {
                        let info = response.data;
                        if(info.errors){
                            (info.errors).map((error)=>(
    ...
    const handelSavingInfo = async () => {
        ...
        await axios.post(AppUrl(`/secure/settings/save-expense-head`),data)
        .then(function (response) {
    ...
    const handleImport = async () => {
        ...
        await axios.post(AppUrl(`/secure/settings/import-expense-heads`),data)
        .then(function (response) {
            let info = response.data;
            if(info.errors){
                (info.errors).map((error)=>(
        ...
        <React.Fragment>
            <Row>
                <Col md={4} sm={12}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Expense Head Information</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            ...
                            <Form.Group className="mb-2" controlId="search">
                                <Form.Label className="m-0">Search <span className="text-danger">*</span></Form.Label>
                                <Select
                                    value={searchFrom.filter(
                                        option => (formData.search && option.value.toString() === (formData.search).toString())
                                    )}
                                    isClearable
                                    options={searchFrom}
                                    onChange={
                            ...
                            <Form.Group className="mb-2" controlId="calculation">
                                <Form.Label className="m-0">Calculation <span className="text-danger">*</span></Form.Label>
                                <Select
                                    value={calculation.filter(
                                        option => (formData.calculation && option.value.toString() === (formData.calculation).toString())
                                    )}
                                    isDisabled={formData.search != "na" ? false : true}
                                    isClearable
                                    options={calculation}
                                    onChange={
                                        option => {                                            
                                            setFormData({
                                                ...formData,
                                                calculation : option ? option.value.toString() : 'n'
                                            })
                                        }
                                    }
                                />
                ...
                <Col md={8} sm={12}>
                    <Card>
                        <Card.Header className="bg-secondary text-white">
                            <Card.Title>Expense Heads List</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            <Row className="mb-2">
                                ...
                                <Col md={4} sm={12}>
                                    ...
                                    <Link to={AppUrl(`/secure/settings/export-expense-heads`)} target="_blank" className="btn btn-success float-end">
                                                    ...
                                                    :(
                                                        (data.infos).map((info,index)=>(
                                                            <tr key={index}>
                                                                <td>
                                                                    {
                                                                        [4,5,6,7].indexOf(info.id) >= 0 || info.deleted_at ? (``) :(
                                                                            <Form.Group>
                                                                                <Form.Check 
                                                                                    type="checkbox" 
            ...
            <Modal show={isImportModalOpen} onHide={handleImportModalClose.bind(this)} backdrop="static" keyboard={false}>
                ...
                <Modal.Body>
                    <strong>
                        ...
                        file must have this fields <br/>
                        (Id,Name, Name (Bangla),search,calculation,Deleted by,Deleted at)
                    </strong>
            ...
            >
                ...
                <Modal.Body>
                    ...
                    <Row>
                        <Col>
                            Search  : {detils.search} <br/>
                            Calculation   : {
                                detils.calculation == 'n' ? 'NA'
                                : detils.calculation == 'i' ? 'Increase (+)'
                                : ('Decrease (-)')
                            } 
                        </Col>
                    </Row>
