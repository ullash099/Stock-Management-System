...
<div class="row">
    <div class="col-12">
        <address class="text-center mb-0">
            ...
            <div class="border border-dark voucher"> {{ __('Income Voucher') }}</div>
        </address>
    </div>
</div>
...
<div class="row">
    ...
    <div class="col text-end">
        ...
        @if (!empty($record->deposit_in_bank_id))
            <p class="m-0"><strong>{{ __('Deposit Bank') }} : </strong>{{ $record->deposit_in_bank->name }}</p>
            <p class="m-0"><strong>{{ __('A/C Name') }} : </strong>{{ $record->deposit_in_bank->account_holder }}</p>
            <p class="m-0"><strong>{{ __('A/C No.') }} : </strong>{{ $record->deposit_in_bank->account_no }}</p>
            @if (!empty($record->deposit_in_bank->bank_address))
                <p class="m-0"><strong>{{ _('Address') }} : </strong>{!! $record->deposit_in_bank->bank_address !!}</p>
            @endif
        @endif
...
<div class="row">
    <div class="col-12">
        ...
        <p>{!! $income_voucher_footer_note ?? '' !!}</p>
    </div>
</div>
@endsection
